# QUALIFIER-MOBILE-01 — Mobilewright Qualification Gate (frozen launch prompt, v2)

Status: FROZEN LAUNCH PROMPT. Authorization: MOBILE-QUALIFICATION-EXEC-AUTH-02 (MOBILE_QUALIFICATION_EXECUTION, issued 2026-09-23 by gilbertosanchez, manifests/mobile-qualification-execution-auth-v2-prepared.yaml). Prompt version: qualifier-mobile-01-mobilewright-gate-v2. Frozen 2026-09-23 by the ORCHESTRATOR, exact byte-for-byte copy of prompts/working/qualifier-mobile-01-mobilewright-gate-v2.md (prompts/README.md, rules 2–5). Starts from the substantive procedure already frozen in v1 (prompts/frozen/qualifier-mobile-01-mobilewright-gate-v1.md, preserved unchanged as historical provenance for the AUTH-01 lineage); the only differences from v1 are this status/header representation and the three launch variables rendered below — no methodological or procedural change.

Variables rendered by the ORCHESTRATOR at launch (the only text that changes between the working and the launched copy):

- `MOBILE-QUALIFICATION-EXEC-AUTH-02` — the human instruction id authorizing execution of the gate.
- `2026-09-23` — its date.
- `qualifier-mobile-01-mobilewright-gate-v2` — the frozen prompt version id under which this run executes.
...
1. **Record the instruction.** In `qualification/mobilewright/README.md`, Environment record, write the instruction to execute: `MOBILE-QUALIFICATION-EXEC-AUTH-02`, `2026-09-23`, relayed by ORCHESTRATOR, prompt version `qualifier-mobile-01-mobilewright-gate-v2`. If the id is missing, stop and record the gap in `qualification/unresolved.md`.
