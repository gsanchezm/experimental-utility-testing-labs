...
## Implementation baseline lock (2026-09-22)

By explicit human decision (LOCAL DEVELOPMENT PREFLIGHT instruction, section 1; verbatim and hashed in `manifests/mobile-qualification-implementation-lock-v1.yaml`), the eleven artifacts named in the table above (ten files plus this README's "Scenario realization" section) are the accepted baseline for the future protocol-valid Mobilewright requalification. All eleven hashes were independently recomputed before the local preflight below and matched exactly (`IMPLEMENTATION_LOCK_HASH_MATCH`); none was edited by the preflight. The record also documents the extraction convention for the README section hash and the human's stated basis for accepting the post-exposure realization changes (implementation/operability changes only; no MQ/MC/N/criteria/attribution/fallback change; no deep-link workaround).

## Local development preflight (LOCAL_DEVELOPMENT_PREFLIGHT; executed 2026-09-22)

Executed under the same instruction (sections 4–16), by the human research lead's original intended next step before any formal gate. Full records: `local-preflight/` (isolated from `output/` above; see `local-preflight/README.md`).

| Item | Value |
|---|---|
| Class | LOCAL_DEVELOPMENT_PREFLIGHT — NON_QUALIFICATION, NON_MEASURED, NOT PART OF N, NOT A WARMUP, NOT PASS/FAIL EVIDENCE, NOT RUNNER-SELECTION EVIDENCE, NOT E03 DATA |
| Android | **COMPLETE** — Samsung Galaxy Z Flip 6 (physical device; not the pinned qualification substrate). MQ1 SUCCESS (catalog reached), MQ2 SUCCESS (product customization, total $15.99→$19.99), MQ3 FAILURE at "checkout screen visible": the deep link's route was delivered (Checkout opened from cold state) but the seeded cart was not rendered (empty-cart state), the same terminal UI state as the quarantined emulator executions — no inference about them is drawn from this diagnostic result (instruction, section 15) |
| iOS | **BLOCKED** — the operator Mac's host simulator service did not respond to any `xcrun simctl` call across six probes (300 s–1985 s); no scenario started; `HOST_SIMULATOR_SERVICE_UNRESPONSIVE`, a pre-scenario infrastructure failure, never a runner or SUT finding |
| Driver | The locked `run-gate-android.sh` is emulator-only by construction and cannot address a physical device; an external driver (kept outside the repository, hash recorded in `local-preflight/android-zflip6/PREFLIGHT-SUMMARY.yaml`) sourced the locked `gate-common.sh` and invoked the locked `harness/run-gate.mjs` unchanged, with the device's adb serial as the runner device id. No repository file was edited. Whether using this driver counts as an implementation change for the future valid gate is left to the human. |
| Must not (honored) | no change to the frozen qualification criteria; no redesign of MQ3 to evade the known MC-07 behavior; no `adb shell am start` or other deep-link workaround; no retry to obtain a pass; no CI/CD dispatch |
| Records | `local-preflight/android-zflip6/` (summary, per-execution sidecar records, harness output imported with device-identifier redaction) and `local-preflight/ios-simulator/` (BLOCKED summary and probe evidence); never gate executions; never placed under `output/` |
