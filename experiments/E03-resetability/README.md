...
| Mobile qualification | **REQUIRED** for `E03-CS001-SUT02-ANDROID` and `E03-CS001-SUT02-IOS`; **NOT_STARTED**; not executed by any operation so far. Mobilewright is the candidate (pinned 0.0.60), Appium 3 the fallback (pinned 3.7.0), Appium 2 never. Frozen-input package prepared 2026-09-22 (`manifests/mobile-qualification-package-v1.yaml`, READY_FOR_HUMAN_AUTHORIZATION: qualification build, runner versions, Android emulator and iOS simulator identities, MQ1–MQ3, N = 10, 60 measured executions, acceptance criteria, fallback trigger, compatibility smoke); executed only by QUALIFIER-MOBILE-01 under `qualification/**` after an explicit MOBILE_QUALIFICATION_EXECUTION authorization, with the working launch prompt `prompts/working/qualifier-mobile-01-mobilewright-gate-v1.md`. The GitHub Actions path (`e03-mobile-qualification.yml`) is prepared and fails closed. SUT-02 must also pass its per-SUT compatibility smoke. | `protocol/mobile-runner-policy-v1.md`; `qualification/` |
...
| E03 START | **NOT AUTHORIZED.** Remaining human approvals: mobile qualification EXECUTION (package READY_FOR_HUMAN_AUTHORIZATION); E03 campaign START; executor prompt freeze and launch. Governance v2 freeze and the public-repository publication mode were decided 2026-09-22. | `manifests/study-manifest.yaml` |
