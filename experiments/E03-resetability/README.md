...
| Mobile qualification | **EXECUTED 2026-09-22** (MOBILE-QUALIFICATION-EXEC-AUTH-01; QUALIFIER-MOBILE-01; `e03-mobile-qualification.yml`, QUALIFICATION runs 35763051534 Android and 35767911878 iOS; 60 measured executions). **Result FAIL**: MQ1/MQ2 10/10 on both platforms, MQ3 10/10 on iOS, MQ3 0/10 on the Android emulator (MC-07 deep link with parameters unsupported under Mobilewright 0.0.60). **Selected mobile runner: Appium 3 (pinned 3.7.0)** by the switch rule (policy section 8; decision record `qualification/mobilewright/README.md`; `manifests/toolchain-manifest.yaml` `mobile_runner_selection`; `protocol/CHANGELOG.md`); Appium 2 never. **Still REQUIRED** before `E03-CS001-SUT02-ANDROID` and `E03-CS001-SUT02-IOS`: the per-SUT compatibility smoke of SUT-02 with Appium 3 (policy section 9), NOT_STARTED, separate explicit human authorization required. Qualification records are tool-qualification records, never SUT evidence. | `protocol/mobile-runner-policy-v1.md`; `qualification/`; `manifests/mobile-qualification-execution-auth-v1.yaml` |
...
| E03 START | **NOT AUTHORIZED.** Remaining human approvals: Appium 3 compatibility smoke EXECUTION for SUT-02 (Android, iOS); E03 campaign START; executor prompt freeze and launch. Governance v2 freeze and the public-repository publication mode were decided 2026-09-22. | `manifests/study-manifest.yaml` |
