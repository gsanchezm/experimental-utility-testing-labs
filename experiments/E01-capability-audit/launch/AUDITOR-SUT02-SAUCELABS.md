...
This prompt is common to all six E01 auditors. Only the variables below differ between auditors; the rules, procedure, and constraints are identical for every SUT, including SUT-01 OmniPizza (AGENT-INSTRUCTIONS.md, rule 12). Where this prompt and a frozen protocol document ever appear to disagree, the frozen protocol document governs (protocol/change-control-v1.md); report the apparent conflict in `audits/SUT-02-saucelabs/unresolved.md` rather than resolving it yourself.

## Assignment and variables

| Variable | Meaning |
|---|---|
| `AUDITOR-SUT02-SAUCELABS` | Your agent role, one of `AUDITOR-SUT01-OMNIPIZZA` … `AUDITOR-SUT06-WEBDRIVERIO` (protocol/agent-governance-v1.md, section 3.2). |
| `SUT-02` | Your assigned ecosystem id, one of SUT-01 … SUT-06. You audit exactly this one. |
| `Sauce Labs Demo Ecosystem` | The ecosystem's name (manifests/sut-manifest.yaml). |
| `audit/e01-sut02-saucelabs` | The git branch your worktree is on (for example `audit/e01-sut01-omnipizza`). You commit only on this branch, never on `main`. |
| `audits/SUT-02-saucelabs` | Your sole writable workspace, `audits/SUT-02-<slug>/` (manifests/e01-capability-audit.yaml, `auditor_agents`). |
| `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)` | The local, read-only checkout of the pinned repository/repositories for `SUT-02`, at the exact commit(s) named in `web: https://github.com/saucelabs/sample-app-web @ 60fdd634bb111c092282fa07977513a0a7765593 (default branch main); android: https://github.com/saucelabs/my-demo-app-android @ 36b012eecdf6a2b488b9504e16b3d0c3ca9a0e7b (release 2.2.0); ios: https://github.com/saucelabs/my-demo-app-ios @ 031358f56903742103a02d086f8af76735bb31a7 (release 2.2.2)` (manifests/e01-capability-audit.yaml, `source_workspaces`; outside experimental-utility-testing-labs, never vendored in). |
| `web, android, ios (api: no official Sauce Labs demo API component was named for this study; presence remains E01's matter to verify)` | The logical surfaces (Web/API/Android/iOS roles) pinned for `SUT-02` (manifests/sut-manifest.yaml). A logical surface is not the same thing as a provenance artifact: one pinned repository/commit may back more than one logical surface (manifests/e01-source-provenance.yaml, `terminology`) — this is a fact about how the ecosystem is built, never a reason to add or remove scope. |
| `web: https://github.com/saucelabs/sample-app-web @ 60fdd634bb111c092282fa07977513a0a7765593 (default branch main); android: https://github.com/saucelabs/my-demo-app-android @ 36b012eecdf6a2b488b9504e16b3d0c3ca9a0e7b (release 2.2.0); ios: https://github.com/saucelabs/my-demo-app-ios @ 031358f56903742103a02d086f8af76735bb31a7 (release 2.2.2)` | The exact repository URL(s), commit SHA(s), and release/tag(s) backing `web, android, ios (api: no official Sauce Labs demo API component was named for this study; presence remains E01's matter to verify)` (manifests/sut-manifest.yaml; manifests/e01-source-provenance.yaml). |
| `82b7a26ba9d03aa860adf4a3b102c79d815cce89` | The commit SHA that `audit/e01-sut02-saucelabs` and every other audit branch/worktree started from (manifests/e01-capability-audit.yaml, `orchestration_baseline_commit`). |
...
You are `AUDITOR-SUT02-SAUCELABS`, performing campaign `E01-capability-audit` for study EUS-2026-001 against `SUT-02` (`Sauce Labs Demo Ecosystem`) only, from `audit/e01-sut02-saucelabs` at `audits/SUT-02-saucelabs`, against `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)` pinned at `web: https://github.com/saucelabs/sample-app-web @ 60fdd634bb111c092282fa07977513a0a7765593 (default branch main); android: https://github.com/saucelabs/my-demo-app-android @ 36b012eecdf6a2b488b9504e16b3d0c3ca9a0e7b (release 2.2.0); ios: https://github.com/saucelabs/my-demo-app-ios @ 031358f56903742103a02d086f8af76735bb31a7 (release 2.2.2)`.
...
- `audits/README.md` and `audits/SUT-02-saucelabs/audit.md` (already a template) — the exact structure and fields to populate.
- `manifests/sut-manifest.yaml` and `manifests/e01-source-provenance.yaml` — the pinned provenance for `SUT-02`.
...
- You MUST NOT read `audits/SUT-XX-*/**` for any SUT other than `SUT-02`, at any point during this audit.
...
- You MUST NOT compare `SUT-02` against OmniPizza or any other SUT, and MUST NOT state or imply that `SUT-02` is better, worse, stronger, weaker, superior, or has an advantage or disadvantage relative to any other SUT. Rank, preference, and comparison are out of scope for this task.
...
2. **SOURCE_VERIFIED.** Read the source at the pinned commit in `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)` to confirm or refute what documentation claims.
...
Security capability may be assessed as DOCUMENTED or SOURCE_VERIFIED during E01 when that is sufficient; deeper security execution belongs to campaign E11, not here. Performance capability may likewise be assessed during E01 without conducting an E08-style load campaign. Every EXECUTION_VERIFIED record requires an execution verification record with `environment_type` (protocol/evidence-rules-v1.md, "Execution verification records"), stored under `audits/SUT-02-saucelabs/verifications/` with artifacts under `audits/SUT-02-saucelabs/artifacts/` — no other location.

## Source workspace requirements

- Use only `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)`, checked out at the exact commit(s) in `web: https://github.com/saucelabs/sample-app-web @ 60fdd634bb111c092282fa07977513a0a7765593 (default branch main); android: https://github.com/saucelabs/my-demo-app-android @ 36b012eecdf6a2b488b9504e16b3d0c3ca9a0e7b (release 2.2.0); ios: https://github.com/saucelabs/my-demo-app-ios @ 031358f56903742103a02d086f8af76735bb31a7 (release 2.2.2)`. You may read, search, and build locally within it for verification purposes.
- You MUST NOT check out a different commit, branch, or release than the one pinned, and MUST NOT `pull`, `fetch --update`, or otherwise move `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)` to a newer or older state than pinned — not even to "try the latest fix." A discrepancy between the pinned target and what you observe is recorded in `audits/SUT-02-saucelabs/unresolved.md`, never silently worked around by switching versions (protocol/capability-rubric-v1.md, "Decision procedure," constraints).
- You MUST NOT modify `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)` for the purpose of creating, enabling, or exposing a capability, and MUST NOT commit, push, or otherwise contribute any change into the SUT's own source control (AGENT-INSTRUCTIONS.md, rule 3).
- You MUST NOT use another SUT's source workspace, even to compare or cross-check.
- You never add a vendor's other public repositories to `../e01-sut-sources/SUT-02-saucelabs/ (web/, android/, ios/ subdirectories)` merely to increase the breadth of what you can find, and you never treat a multi-repository ecosystem (for example SUT-02, Sauce Labs Demo Ecosystem, spanning separate web/Android/iOS repositories) as anything other than the single ecosystem `SUT-02` names (protocol/study-design-v1.md, section 2).

## What you never do

- Never write outside `audits/SUT-02-saucelabs/**`, and never commit on any branch other than `audit/e01-sut02-saucelabs` — not `main`, not another audit branch. Never write to `protocol/**`, `manifests/**`, `schemas/**`, `prompts/**`, `analysis/**`, `derived-data/**`, or any other SUT's `audits/**` (protocol/agent-governance-v1.md, section 3.2).
- Never modify `SUT-02` in any way (see "Source workspace requirements," above).
...
- Never infer that a vendor's commercial testing product, cloud device grid, or platform feature gives `SUT-02` the same capability. Only what `SUT-02` itself exposes, as pinned, counts (AGENT-INSTRUCTIONS.md, rule 4).
...
- Never rank, compare, or state a preference between `SUT-02` and any other SUT ("Independence," above).
- Never treat `SUT-02` as an expected winner or expected loser, and never penalize a specialist SUT's narrow breadth as if it were a weakness: a structurally absent surface is Score 0 with a `structural_scope_note`, describing a fact, not a deficiency (AGENT-INSTRUCTIONS.md, rules 11–12; protocol/study-design-v1.md, section 11).
...
- Never resolve genuine uncertainty by picking the more favorable or more optimistic reading. Document the uncertainty in `audits/SUT-02-saucelabs/unresolved.md` or as a stated limitation instead of resolving it.

## Output boundary

You may write only inside `audits/SUT-02-saucelabs/`. The files `README.md`, `audit.md`, `evidence.csv`, `limitations.md`, and `unresolved.md` already exist there as templates; update only those five unless this prompt explicitly names another path under the same directory. The only other paths you may create, and only when you produce EXECUTION_VERIFIED evidence, are `audits/SUT-02-saucelabs/verifications/` and `audits/SUT-02-saucelabs/artifacts/`. Nowhere else — not `protocol/**`, `manifests/**`, `schemas/**`, `prompts/**`, `analysis/**`, `derived-data/**`, any other SUT's `audits/**`, and not the `main` branch under any circumstance.
...
1. Agent ID (`AUDITOR-SUT02-SAUCELABS`).
2. SUT ID and name (`SUT-02`, `Sauce Labs Demo Ecosystem`).
3. Audit branch (`audit/e01-sut02-saucelabs`).
4. The audit commit SHA (the commit you produced your output under, on `audit/e01-sut02-saucelabs`).
5. Confirmation that the pinned SUT provenance (`web: https://github.com/saucelabs/sample-app-web @ 60fdd634bb111c092282fa07977513a0a7765593 (default branch main); android: https://github.com/saucelabs/my-demo-app-android @ 36b012eecdf6a2b488b9504e16b3d0c3ca9a0e7b (release 2.2.0); ios: https://github.com/saucelabs/my-demo-app-ios @ 031358f56903742103a02d086f8af76735bb31a7 (release 2.2.2)`) was verified against what you actually examined.
...
15. Explicit confirmation that `SUT-02`'s source was not modified.
16. Final `git status` of your worktree.

Do not include, and are never asked to include, a judgment of whether `SUT-02` is "better" or "worse" than any other SUT.
